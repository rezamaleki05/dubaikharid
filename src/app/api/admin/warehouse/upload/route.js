...
export async function POST(request) {
  ...
  const pathname = `warehouse/${new Date().getUTCFullYear()}/${randomUUID()}.${validation.extension}`;
  try {
    ...
    await logAdminActivity({
      adminId: admin.id,
      action: 'WAREHOUSE_IMAGE_UPLOADED',
      entityType: 'WarehouseItemImage',
    ...
    return NextResponse.json({ error: 'آپلود تصویر انبار با خطا مواجه شد.' }, { status: 500 });
  }
}
