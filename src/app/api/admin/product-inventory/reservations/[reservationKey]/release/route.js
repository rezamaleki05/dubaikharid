...
import { releaseProductInventoryReservation } from '@/lib/productInventoryService';
...
export async function POST(request, { params }) {
  ...
  try {
    const reservation = await releaseProductInventoryReservation(prisma, { reservationKey: reservationKey.value, adminId: admin.id });
    await logAdminActivity({
      adminId: admin.id,
      action: 'PRODUCT_INVENTORY_RESERVATION_RELEASED',
    ...
    return productInventoryApiError(error, 'Product inventory reservation release failed:');
  }
}
