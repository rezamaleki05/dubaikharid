...
import {
  adminPaymentInclude,
  applySuccessfulPaymentOrderEffects,
...
export async function POST(request) {
  ...
  try {
    const payment = await prisma.$transaction(async tx => {
      ...
      if (parsed.data.status === 'success') await applySuccessfulPaymentOrderEffects(tx, order.id);
      return payment;
    ...
    if (payment.status === 'success' && ['pending', 'pricing'].includes(payment.order?.status)) await logAdminActivity({ adminId: admin.id, action: 'ORDER_STATUS_CHANGED', entityType: 'Order', entityId: payment.orderId, metadata: { previousStatus: payment.order.status, newStatus: 'paid', source: 'payment_confirmation' }, request });
