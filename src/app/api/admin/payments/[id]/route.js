...
import { getPaymentConfirmationError, PAYMENT_CONFIRMATION_MODE_SET } from '@/lib/paymentConfirmation';
...
export async function PATCH(request, { params }) {
  ...
  if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).some(key => !['status', 'notes', 'rejectionReason', 'confirmationMode'].includes(key))) return NextResponse.json({ error: 'بدنه درخواست معتبر نیست.' }, { status: 400 });
  if (!['success', 'failed', 'refunded'].includes(body.status)) return NextResponse.json({ error: 'وضعیت پرداخت معتبر نیست.' }, { status: 400 });
  if (Object.hasOwn(body, 'confirmationMode') && (body.status !== 'success' || typeof body.confirmationMode !== 'string' || !PAYMENT_CONFIRMATION_MODE_SET.has(body.confirmationMode))) return NextResponse.json({ error: 'روش تأیید پرداخت معتبر نیست.' }, { status: 400 });
  ...
  try {
    const result = await prisma.$transaction(async tx => {
      ...
      if (body.status === 'success') {
        const confirmationError = getPaymentConfirmationError({ method: current.method, hasReceipt: Boolean(current.receiptBlobPathname), confirmationMode: body.confirmationMode, notes });
        if (confirmationError) throw Object.assign(new Error(confirmationError), { status: ['MANUAL_NOTE_REQUIRED', 'MANUAL_NOTE_TOO_LONG'].includes(confirmationError) ? 400 : 409 });
      ...
      const payment = await tx.payment.update({ where: { id }, data: { status: body.status, notes: Object.hasOwn(body, 'notes') ? notes || null : undefined, rejectionReason: body.status === 'failed' ? rejectionReason : null, paidAt: body.status === 'success' ? new Date() : current.paidAt, confirmedById: body.status === 'success' ? admin.id : current.confirmedById }, include: adminPaymentInclude });
      if (body.status === 'success') await applySuccessfulPaymentOrderEffects(tx, current.orderId);
      return { payment, changed: true, previousStatus: current.status, orderPreviousStatus: current.order?.status || null, confirmationMode: body.status === 'success' ? body.confirmationMode || null : null };
    }, { isolationLevel: 'Serializable' });
    if (result.changed) await logAdminActivity({ adminId: admin.id, action: result.payment.status === 'success' ? 'PAYMENT_MARKED_PAID' : result.payment.status === 'refunded' ? 'PAYMENT_REFUNDED' : 'PAYMENT_FAILED', entityType: 'Payment', entityId: id, metadata: { paymentId: id, previousStatus: result.previousStatus, newStatus: result.payment.status, orderId: result.payment.orderId, amount: result.payment.amount.toString(), currency: result.payment.currency, method: result.payment.method, confirmationMode: result.confirmationMode || null }, request });
    ...
    const messages = { NOT_FOUND: ['پرداخت پیدا نشد.', 404], INVALID_TRANSITION: ['تغییر وضعیت پرداخت مجاز نیست.', 409], INVALID_CONFIRMATION_MODE: ['روش تأیید با وضعیت این پرداخت همخوانی ندارد.', 409], UNTRUSTED_GATEWAY: ['تأیید دستی پرداخت آنلاین بدون پاسخ معتبر درگاه مجاز نیست.', 409], RECEIPT_REQUIRED: ['برای تأیید بر اساس رسید، ابتدا باید رسید مشتری ثبت شود.', 409], MANUAL_NOTE_REQUIRED: ['ثبت یادداشت بررسی بانکی برای تأیید دستی الزامی است.', 400], MANUAL_NOTE_TOO_LONG: ['یادداشت بررسی بانکی نباید بیش از ۱۰۰۰ نویسه باشد.', 400], ORDER_TOTAL_MISSING: ['مبلغ معتبر سفارش در دسترس نیست.', 409], AMOUNT_MISMATCH: ['مبلغ پرداخت با مانده معتبر سفارش مطابقت ندارد.', 409], ORDER_CANCELLED: ['سفارش مرتبط لغو شده است.', 409] };
