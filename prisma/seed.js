...
async function main() {
  ...
  for (const b of brandsData) {
    const matchingCat = categories.find(c => c.name === b.cat);
  ...
  const fashionCategory = categories.find(c => c.name === 'مد و پوشاک');
  ...
  if (nikeBrand) {
    const p2 = await prisma.product.create({
      data: {
        ...
        categoryId: categories.find(c => c.name === 'ورزشی ( اسپورت )')?.id || null,
        priceAed: 529,
        weight: 0.9,
        gender: 'unisex',
        isBestSeller: true,
